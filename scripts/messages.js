...
export function renderMessages() {
  ...
  // --- Alt-to-Scroll Global Initialization ---
  if (!document.getElementById("messages-alt-styles")) {
    const style = document.createElement("style");
    style.id = "messages-alt-styles";
    style.innerHTML = `
      .message-body-content {
          position: relative;
          overflow: hidden !important; /* Force hide unless alt is pressed */
          transition: background 0.3s, border-color 0.3s;
      }
      .message-body-content.has-overflow::after {
          content: '';
          position: absolute;
      ...
      .alt-scroll-badge {
          ...
          background: rgba(227, 185, 56, 0.95);
          color: #000;
          padding: 6px 14px;
          border-radius: 20px;
          font-size: 0.8rem;
      ...
      .message-body-content.has-overflow:hover .alt-scroll-badge {
          opacity: 1;
          transform: translateX(-50%) translateY(0);
      }
      
      /* Active Alt Mode */
      body.alt-scroll-active .message-body-content.has-overflow:hover {
          overflow-y: auto !important;
          background: rgba(0,0,0,0.4);
          border-right-color: var(--metallic-gold);
      }
      body.alt-scroll-active .message-body-content.has-overflow:hover::after,
      body.alt-scroll-active .message-body-content.has-overflow:hover .alt-scroll-badge {
          opacity: 0 !important;
      }
    `;
    document.head.appendChild(style);

    const toggleAltMode = (e) => {
      if (e.key === "Alt" || e.keyCode === 18) {
        if (e.type === "keydown")
          document.body.classList.add("alt-scroll-active");
        if (e.type === "keyup")
          document.body.classList.remove("alt-scroll-active");
      }
    };
    window.addEventListener("keydown", toggleAltMode);
    window.addEventListener("keyup", toggleAltMode);
    window.addEventListener("blur", () =>
      document.body.classList.remove("alt-scroll-active"),
    );
  }
  // --- End Alt Initialization ---

  let isDragLocked = Storage.get("message_drag_locked") === true;
  let currentHeight = Storage.get("message_card_height") || "auto";
  let currentLayout = Storage.get("message_grid_layout") || "4";

  const header = document.createElement("div");
  header.className = "section-header";
  header.innerHTML = `
            <div style="display:flex; flex-direction:column; gap:8px;">
                <h1 class="section-title" style="margin:0;">قوالب الرسائل</h1>
            </div>
            <div style="display:flex; gap:10px; align-items: center; flex-wrap: wrap; justify-content: flex-end;">
                
                <!-- Custom Horizontal Scroll Slider -->
                <div class="custom-horizontal-scroller" style="width: 150px; height: 30px; background: rgba(0,0,0,0.2); border-radius: 30px; border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(10px); position: relative; margin-left: 10px; display: flex; align-items: center; padding: 0 4px; overflow: hidden; touch-action: none; cursor: ew-resize;" title="اسحب للتمرير العمودي">
                    <div class="scroll-track" style="width: 100%; height: 4px; background: rgba(255,255,255,0.1); border-radius: 4px; position: relative; pointer-events: none;">
                        <div class="scroll-thumb" style="width: 22px; height: 22px; background: var(--metallic-gold); border-radius: 50%; position: absolute; right: 0; top: 50%; transform: translateY(-50%); box-shadow: 0 2px 5px rgba(0,0,0,0.3); pointer-events: none; transition: transform 0.1s ease-out; z-index: 2;"></div>
                    </div>
                </div>
                
                <div style="width: 1px; height: 25px; background: rgba(255,255,255,0.1); margin: 0 5px;"></div>

                <div class="view-controls layout-controls" style="display:flex; gap:4px; background:rgba(0,0,0,0.2); padding:4px; border-radius:30px; border:1px solid rgba(255,255,255,0.1); backdrop-filter: blur(10px);">
                    <button id="layout-1" title="عمود واحد" class="btn-icon" style="width:30px; height:30px; border-radius:50%; border:none; background:transparent; color:rgba(255,255,255,0.5); cursor:pointer; display:flex; align-items:center; justify-content:center; transition: all 0.2s;"><i class="fa-solid fa-bars" style="font-size: 0.85rem;"></i></button>
                    <button id="layout-2" title="عمودين" class="btn-icon" style="width:30px; height:30px; border-radius:50%; border:none; background:transparent; color:rgba(255,255,255,0.5); cursor:pointer; display:flex; align-items:center; justify-content:center; transition: all 0.2s;"><i class="fa-solid fa-grip-vertical" style="font-size: 0.85rem;"></i></button>
                    <button id="layout-4" title="4 أعمدة" class="btn-icon active" style="width:30px; height:30px; border-radius:50%; border:none; background:var(--metallic-gold); color:#000; cursor:pointer; display:flex; align-items:center; justify-content:center; transition: all 0.2s;"><i class="fa-solid fa-grip" style="font-size: 0.85rem;"></i></button>
                </div>

                <div style="width: 1px; height: 25px; background: rgba(255,255,255,0.1); margin: 0 5px;"></div>

                <div class="view-controls height-controls" style="display:flex; gap:4px; background:rgba(0,0,0,0.2); padding:4px; border-radius:30px; border:1px solid rgba(255,255,255,0.1); backdrop-filter: blur(10px);">
  ...
  const layoutBtns = {
    1: header.querySelector("#layout-1"),
    2: header.querySelector("#layout-2"),
    4: header.querySelector("#layout-4"),
  };

  const updateLayout = (l) => {
    currentLayout = l;
    Storage.set("message_grid_layout", l);
    Object.values(layoutBtns).forEach((btn) => {
      btn.style.background = "transparent";
      btn.style.color = "rgba(255,255,255,0.5)";
      btn.classList.remove("active");
    });
    const activeBtn = layoutBtns[l];
    if (activeBtn) {
      activeBtn.style.background = "var(--metallic-gold)";
      activeBtn.style.color = "#000";
      activeBtn.classList.add("active");
    ...
    if (templatesGrid) {
      if (l === "1") {
        ...
        templatesGrid.style.gridTemplateColumns = "repeat(4, 1fr)";
      }
    }
  };

  layoutBtns["1"].onclick = () => updateLayout("1");
  layoutBtns["2"].onclick = () => updateLayout("2");
  layoutBtns["4"].onclick = () => updateLayout("4");

  setTimeout(() => updateLayout(currentLayout), 0);

  const lockBtn = header.querySelector("#lock-drag-btn");
  const updateLockUI = () => {
    lockBtn.innerHTML = isDragLocked
      ? '<i class="fa-solid fa-lock"></i>'
      : '<i class="fa-solid fa-lock-open"></i>';
    lockBtn.style.color = isDragLocked ? "#ff4d4d" : "inherit";
    lockBtn.style.borderColor = isDragLocked
      ? "#ff4d4d"
  ...
  // --- Custom Horizontal Scroll Slider Logic ---
  const scrollerArea = header.querySelector(".custom-horizontal-scroller");
  const thumb = scrollerArea.querySelector(".scroll-thumb");
  ...
  const getScrollContainer = () => {
    ...
    return document.documentElement; // fallback to html/window for desktop
  };

  // Utility to recalculate and sync thumb position based on scroll
  const syncThumbFromScroll = () => {
    if (!document.contains(scrollerArea)) {
      window.removeEventListener("scroll", syncThumbFromScroll);
      const contentArea = document.querySelector(".content-area");
      if (contentArea)
        contentArea.removeEventListener("scroll", syncThumbFromScroll);
      return;
    }
    ...
    // Percentage from 0 (top) to 1 (bottom)
    const perc = Math.max(0, Math.min(1, scrollTop / maxScroll));

    // The thumb moves from right (0px) to left (-maxThumbScroll px)
    // Left direction in transform is negative
    const maxThumbScroll = scrollerArea.clientWidth - 8 - thumb.clientWidth; // 8px padding
    const translateX = -(perc * maxThumbScroll);

    thumb.style.transition = "none"; // remove transition for smooth scrolling sync
    thumb.style.transform = `translateY(-50%) translateX(${translateX}px)`;
  };

  // Dragging Logic
  let isDraggingScroller = false;
  let startX = 0;
  ...
  const handleDragStart = (e) => {
    ...
    const startScrollTop =
      container === document.documentElement
        ? window.scrollY
        : container.scrollTop;
    const maxThumbScroll = scrollerArea.clientWidth - 8 - thumb.clientWidth;
    const scrollHeight = container.scrollHeight;
    const clientHeight = container.clientHeight;
    const maxScroll = Math.max(1, scrollHeight - clientHeight); // Prevent divide by 0

    // Calculate starting progress based on current scroll to avoid jumping
    thumb.dataset.startPerc = Math.max(
      0,
      Math.min(1, startScrollTop / maxScroll),
    );

    scrollerArea.style.background = "rgba(255, 215, 0, 0.1)";
    thumb.style.boxShadow = "0 0 10px rgba(227, 185, 56, 0.5)";
  ...
  const handleDragMove = (e) => {
    if (!isDraggingScroller) return;
    const x = e.type.includes("mouse") ? e.clientX : e.touches[0].clientX;

    // Delta X: how far we moved horizontally.
    // Negative deltaX means moving cursor left.
    const deltaX = x - startX;
    const maxThumbScroll = scrollerArea.clientWidth - 8 - thumb.clientWidth;

    // Perc Change: Since right is 0 and left is -maxThumbScroll,
    // moving left (negative deltaX) should INCREASE percentage.
    // moving right (positive deltaX) should DECREASE percentage.
    const percChange = -(deltaX / maxThumbScroll);

    let newPerc = parseFloat(thumb.dataset.startPerc) + percChange;
    newPerc = Math.max(0, Math.min(1, newPerc));

    // Calculate scroll target and execute
    const container = getScrollContainer();
    const scrollHeight = container.scrollHeight;
    const clientHeight = container.clientHeight;
    const maxScroll = scrollHeight - clientHeight;
    const targetScrollTop = newPerc * maxScroll;
  ...
  const handleDragEnd = () => {
    ...
    scrollerArea.style.background = "rgba(0,0,0,0.2)";
  ...
  // Attach scroller drag events
  scrollerArea.addEventListener("mousedown", handleDragStart);
  scrollerArea.addEventListener("touchstart", handleDragStart, {
    passive: false,
  });
  ...
  // Attach syncing from native scrolling
  // setTimeout used to let the DOM settle first
  setTimeout(() => {
    window.addEventListener("scroll", syncThumbFromScroll, { passive: true });
    const contentArea = document.querySelector(".content-area");
    if (contentArea) {
      contentArea.addEventListener("scroll", syncThumbFromScroll, {
        passive: true,
      });
    }
    syncThumbFromScroll(); // initial sync
  }, 100);
  // --- End Custom Slider Logic ---
  ...
  templatesGrid.className = "messages-templates-grid";
  templatesGrid.style.display = "grid";
  // Base initial values. These will be overridden by the layout toggle right after initialization.
  templatesGrid.style.gridTemplateColumns = "repeat(4, 1fr)";
  templatesGrid.style.gap = "1.5rem";
  templatesGrid.style.alignContent = "start";
  templatesGrid.style.transition = "all 0.3s ease";
  ...
  function renderCategories() {
    ...
    categories
      ...
      .forEach((cat) => {
        ...
        li.style.color = isActive
          ? "var(--metallic-gold)"
          : "var(--text-secondary)";
        li.style.background = isActive
          ? "rgba(255, 215, 0, 0.1)"
          : "rgba(255,255,255,0.05)";
        li.style.borderColor = isActive
          ? "var(--metallic-gold)"
          : "rgba(255,255,255,0.1)";
    ...
    catSidebar.ondrop = (e) => {
      ...
      const currentOrderNames = [
        ...catSidebar.querySelectorAll("[data-name]"),
      ].map((el) => el.dataset.name);

      // Reconstruct the categories array based on the DOM order
      const newOrder = currentOrderNames
        .map((name) => {
          if (name === "الكل")
            return { id: "all", name: "الكل", icon: "fa-folder" };
    ...
    catSidebar.querySelector("#manage-cats-btn").onclick = () =>
      openCategoryManager();
  }

  function getDragAfterElementHorizontal(container, x) {
    const draggableElements = [
      ...container.querySelectorAll('[draggable="true"]:not(.dragging-cat)'),
    ];
  ...
  function renderTemplates() {
    ...
    filtered.forEach((msg, idx) => {
      if (typeof msg.order === "undefined") msg.order = idx;
    });
    ...
    if (filtered.length === 0) {
      templatesGrid.innerHTML = `
                    <div style="column-span: all; grid-column: 1 / -1; text-align: center; padding: 4rem 2rem; color: var(--text-secondary); background: rgba(255,255,255,0.02); border-radius: 16px; border: 1px dashed rgba(255,255,255,0.1);">
    ...
    filtered.forEach((msg, index) => {
      ...
      if (msg.isPinned) {
        ...
        card.style.background =
          "linear-gradient(145deg, rgba(255, 215, 0, 0.05) 0%, rgba(255,255,255,0.02) 100%)";
      } else {
        card.style.background =
      ...
      card.innerHTML = `
                    ...
                    ">
                        <div class="msg-text-inner" style="height: 100%;">${msg.body}</div>
                        <div class="alt-scroll-badge"><i class="fa-solid fa-hand-pointer"></i> اضغط Alt للتمرير</div>
                    </div>
      ...
      card.addEventListener("dragend", () => {
        ...
        document.querySelectorAll(".message-card").forEach((c) => {
          if (!msg.isPinned)
            c.style.borderTop = "1px solid rgba(255,255,255,0.05)";
        });
      });
    ...
    // Detect overflow after rendering
    setTimeout(() => {
      templatesGrid
        .querySelectorAll(".message-body-content")
        .forEach((bodyContent) => {
          const inner = bodyContent.querySelector(".msg-text-inner");
          if (inner && inner.scrollHeight > bodyContent.clientHeight + 2) {
            bodyContent.classList.add("has-overflow");
          } else {
            bodyContent.classList.remove("has-overflow");
          }
        });
    }, 50);
    ...
    templatesGrid.ondrop = (e) => {
      ...
      const ids = [...templatesGrid.querySelectorAll(".message-card")].map(
        (c) => c.dataset.id,
      );
      const allMessages = Storage.getList("messages_data");
  ...
  function getDragAfterElement(container, x, y) {
    const draggableElements = [
      ...container.querySelectorAll(".message-card:not(.dragging)"),
    ];
                ...
                <form id="msg-form" style="display:grid; gap:1.2rem;">
                    ...
                    <div>
                        <select id="m-category" class="glass-input" style="width:100%;">
                            ${(Storage.getList("message_categories") || ["عام"])
                              .map(
                                (cat) => `
