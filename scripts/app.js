  ...
  if (routes[hash]) {
    try {
      ...
      import("./core.js").then((m) =>
        m.Notifications.error("فشل تحميل القسم المطلوب"),
      );
    ...
    if (routeId !== currentRouteId) return;
    contentArea.innerHTML =
      '<h2 class="page-enter" style="text-align:center; padding-top:4rem;">404 - Page Not Found</h2>';
  }
...
function initApp() {
  ...
  if (settings.cursor) initCursor(settings.cursorStyle || "gold-ring");
  else initCursor("default");
  window.clickEffectType = settings.clickEffect || "none";

  if (settings.fontSize)
    document.documentElement.style.fontSize =
      16 * (settings.fontSize / 100) + "px";
  ...
  if (settings.themeColor) {
    ...
    const r = parseInt(settings.themeColor.slice(1, 3), 16),
      g = parseInt(settings.themeColor.slice(3, 5), 16),
      b = parseInt(settings.themeColor.slice(5, 7), 16);
    document.documentElement.style.setProperty(
      "--gold-rgb",
      `${r}, ${g}, ${b}`,
    );
  ...
  if (settings.glassBlur) {
    ...
    document
      ...
      .forEach(
        (el) => (el.style.backdropFilter = `blur(${settings.glassBlur}px)`),
      );
  }
...
window.applySidebarOrder = () => {
  const loaded = JSON.parse(localStorage.getItem("app_settings")) || {};
  const sidebarOrder = loaded.sidebarOrder || [
    "leads",
    "almdrasa",
    "messages",
    "notes",
...
function createLockScreen() {
  ...
  const handleUnlock = () => {
    ...
    const passwordMap = {
      ...
      "ahmad@leadsync": "أحمد كمال",
    };

    if (passwordMap[password]) {
      const userName = passwordMap[password];
      const greeting =
        password === "leadsync-guest"
          ? `أهلاً بك ${userName}`
          : `المشرف الحالي: ${userName}`;
