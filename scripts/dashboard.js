...
export async function renderDashboard() {
  ...
  try {
    ...
    const currentUserGreeting =
      sessionStorage.getItem("currentUser") || "الـ HQ";
    const rawName = currentUserGreeting.includes(":")
      ? currentUserGreeting.split(":")[1].trim()
      : currentUserGreeting;
    ...
    header.innerHTML = ` <div class="welcome-card"><div class="user-profile"><div class="welcome-info"><span class="welcome-label">مرحباً بك مجدداً</span><h1 class="user-name">${rawName}
    ...
    setTimeout(async () => {
      const quranBtn = header.querySelector("#quran-btn");
      if (quranBtn) {
        quranBtn.onclick = async () => {
          const { QuranReader } = await import("./quran-reader.js");
          if (!window.quranReader) {
            window.quranReader = new QuranReader();
          }
          window.quranReader.show();
        };
      }
    }, 100);
    ...
    try {
      ...
      h = h ? h : 12;
      const m = d.getMinutes();
      header.querySelector("#clock-h").innerText = h
        .toString()
        .padStart(2, "0");
      header.querySelector("#clock-m").innerText = m
        .toString()
        .padStart(2, "0");
    ...
    msgCard.innerHTML = ` <h3 style="margin-bottom: 1rem;"><i class="fa-solid fa-clock-rotate-left gold-text"></i>آخر رسالة</h3>${
      lastMessage
        ? ` <div style="background: rgba(0,0,0,0.3); padding: 1rem; border-radius: 8px;"><h4 style="color: var(--metallic-gold); margin-bottom: 0.5rem;">${
      ...
      ${(lastMessage.body || "").length > 100 ? "..." : ""}
        ...
        : `<p style="color: var(--text-secondary);">لا توجد رسائل بعد.</p><button class="btn btn-primary" style="margin-top: 1rem;" onclick="location.hash='#messages'">إنشاء أول رسالة</button>`
    }
    ...
    remCard.innerHTML = ` <h3 style="margin-bottom: 1rem;"><i class="fa-solid fa-bell gold-text"></i>التذكير القادم</h3>${
      nextReminder
        ? ` <div style="background: rgba(0,0,0,0.3); padding: 1rem; border-radius: 8px; border-left: 3px solid var(--metallic-gold);"><h4 style="font-size: 1.1rem; margin-bottom: 0.5rem;">${
            nextReminder.title
          }
        ...
        : `<p style="color: var(--text-secondary);">لا توجد تذكيرات قادمة.</p><button class="btn btn-primary" style="margin-top: 1rem;" onclick="location.hash='#reminders'">إضافة تذكير</button>`
    }
    ...
    const renderActions = () => {
        ...
        showingExternal
          ...
          : savedActions
              .map((key) => {
                ...
                return `<button class="btn btn-glass" onclick="location.hash='#${key}'"> <i class="fa-solid ${action.icon}"></i> ${action.label}
      ...
      actionCard.querySelector(".edit-actions-btn").onclick = () => {
        ...
        modal.innerHTML = ` <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem;"><h3>تخصيص الإجراءات السريعة</h3><button class="btn btn-glass close-modal" style="padding:5px 10px;"><i class="fa-solid fa-xmark"></i></button></div><div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; flex:1; overflow-y:auto; padding-right:5px;">${Object.keys(
          ...
          ">
<i class="fa-solid ${allPossibleActions[key].icon}
...
<span style="font-size:0.9rem; color:${
              currentActions.includes(key) ? "#fff" : "rgba(255,255,255,0.6)"
            }
      ...
      ">${allPossibleActions[key].label}</span>
        ...
        overlay.style.cssText =
          "position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.8); z-index:9999; backdrop-filter:blur(8px);";
...
${timeStr}
      ...
      if (timeEl)
        timeEl.innerText = `${isOldUI ? "مخزنة: " : "مباشر: "}
      ...
      radio.volume
        ...
        const updateUI = (state, reciter) => {
          ...
          if (infoBox) {
            infoBox.innerText =
              reciter.bio ||
              "استمتع بتلاوة القرآن الكريم من مختلف القراء والإذاعات.";
          ...
          if (mirrorBtn) {
            mirrorBtn.style.display =
              reciter.mirrors && reciter.mirrors.length > 1 ? "flex" : "none";
          }

          const muteBtnIcon = radioContainer.querySelector(".mute-btn i");

          if (muteBtnIcon) {
            muteBtnIcon.className = radio.muted
              ? "fa-solid fa-volume-xmark"
              : "fa-solid fa-volume-high";
            muteBtnIcon.parentElement.style.color = radio.muted
          ...
          if (state === "playing") {
            ...
            statusTag.querySelector(".status-text").innerText =
              "جاري التحميل...";
        ...
        radioContainer.querySelector("#refresh-stream").onclick = () =>
          radio.refresh();
        radioContainer.querySelector("#switch-mirror").onclick = () =>
          radio.nextMirror();
...
width:100%; padding:12px; background:rgba(255, 255, 255, 0.05);
    ...
    ">
<div class="reciter-list" style="flex:1; overflow-y:auto; padding-right:5px;"> ${radio.reciters
            .map(
              (r, idx) => ` <div class="reciter-item" data-idx="${idx}" style="
...
<span>${r.name}
          ...
          idx === radio.currentReciterIndex
            ? '<i class="fa-solid fa-check gold-text"></i>'
            : ""
        }
          ...
          const overlay = document.createElement("div");
          overlay.style.cssText =
            "position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:9999; backdrop-filter:blur(5px);";
        ...
        const animateWave = () => {
          ...
          if (radio.state === "playing") {
            ...
            if (freqData) {
              ...
              deg) brightness(${1 + currentBarHeights[i] / 250}
                ...
                if (peak) {
                  peak.style.transform = `translateY(${-peakValues[i] / 2}
    ...
    errDiv.innerHTML = `<h3 style="color:red">Dashboard Error: ${error.message}
