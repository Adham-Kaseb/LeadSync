...
export async function renderDashboard() {
  ...
  try {
    ...
    if (reminders.length > 0) {
      nextReminder=reminders .filter((r)=> new Date(r.dateTime) > now) .sort((a, b)=> new Date(a.dateTime) - new Date(b.dateTime))[0];
    }
    ...
    header.innerHTML=` <div class="welcome-card"><div class="user-profile"><div class="welcome-info"><span class="welcome-label">مرحباً بك مجدداً</span><h1 class="user-name">${
      rawName
    }

    </h1><div class="date-badge-container"><div class="date-badge gold-badge"><i class="fa-solid fa-moon"></i><span>${
      hijriDate
    }

    </span></div><div class="date-badge"><i class="fa-regular fa-calendar"></i><span>${
      gregorianDate
    }
    ...
    const updateTime=()=> {
      ...
      if (elH && elH.innerText !==hStr) {
        elH.innerText=hStr;
        elH.classList.add("flip");
        setTimeout(()=> elH.classList.remove("flip"), 500);
      }

      if (elM && elM.innerText !==mStr) {
        elM.innerText=mStr;
        elM.classList.add("flip");
        setTimeout(()=> elM.classList.remove("flip"), 500);
      }
    ...
    // Media query override for very small screens via CSS is better, 
    // but we can adjust min-width here to be more adaptive
    ...
    msgCard.innerHTML=` <h3 style="margin-bottom: 1rem;"><i class="fa-solid fa-clock-rotate-left gold-text"></i>آخر رسالة</h3>${
      lastMessage ? ` <div style="background: rgba(0,0,0,0.3); padding: 1rem; border-radius: 8px;"><h4 style="color: var(--metallic-gold); margin-bottom: 0.5rem;">${
        lastMessage.title || "Untitled"
      }

      </h4><p style="color: var(--text-secondary); font-size: 0.9rem; line-height: 1.4;">${
        (lastMessage.body || ""
        ).substring(0, 100)
      }
      ...
      </p><div style="margin-top: 0.8rem; font-size: 0.8rem; opacity: 0.7;"><i class="fa-solid fa-feather"></i>${
        Utils.formatDate(lastMessage.timestamp)
      }

      </div></div><button class="btn btn-glass" style="margin-top: 1rem; width: 100%;" onclick="location.hash='#messages'">فتح الرسائل</button>` : `<p style="color: var(--text-secondary);">لا توجد رسائل بعد.</p><button class="btn btn-primary" style="margin-top: 1rem;" onclick="location.hash='#messages'">إنشاء أول رسالة</button>`
    }
    ...
    remCard.innerHTML=` <h3 style="margin-bottom: 1rem;"><i class="fa-solid fa-bell gold-text"></i>التذكير القادم</h3>${
      nextReminder ? ` <div style="background: rgba(0,0,0,0.3); padding: 1rem; border-radius: 8px; border-left: 3px solid var(--metallic-gold);"><h4 style="font-size: 1.1rem; margin-bottom: 0.5rem;">${
        nextReminder.title
      }

      </h4><p style="font-size: 1.2rem; font-weight: 700; color: var(--text-secondary);">${
        new Date(nextReminder.dateTime).toLocaleTimeString("ar-EG", {
            hour: "2-digit",
            minute: "2-digit",
          }
      ...
      </p><p style="font-size: 0.9rem; opacity: 0.8;">${
        new Date(nextReminder.dateTime).toLocaleDateString("ar-EG")
      }

      </p></div><button class="btn btn-glass" style="margin-top: 1rem; width: 100%;" onclick="location.hash='#reminders'">إدارة التذكيرات</button>` : `<p style="color: var(--text-secondary);">لا توجد تذكيرات قادمة.</p><button class="btn btn-primary" style="margin-top: 1rem;" onclick="location.hash='#reminders'">إضافة تذكير</button>`
    }
    ...
    const allPossibleActions= {
      leads: {
        label: "عميل جديد", icon: "fa-user-plus"
      }

      ,
      notes: {
        label: "ملاحظة جديدة", icon: "fa-plus"
      }

      ,
      calculator: {
        label: "الخصومات", icon: "fa-calculator"
      }

      ,
      calendar: {
        label: "التقويم", icon: "fa-calendar-days"
      }

      ,
      settings: {
        label: "الإعدادات", icon: "fa-gear"
      }

      ,
      messages: {
        label: "الرسائل", icon: "fa-comment-dots"
      }

      ,
      almdrasa: {
        label: "المدرسة", icon: "fa-school"
      }
      ...
      ,
      reminders: {
        label: "التذكيرات", icon: "fa-bell"
      }

      ,
      articles: {
        label: "المقالات", icon: "fa-newspaper"
      }

      ,
      tvmode: {
        label: "التلفاز", icon: "fa-tv"
      }

      ,
      whatsapp: {
        label: "واتساب", icon: "fa-square-phone"
      }

      ,
      meetings: {
        label: "الاجتماعات", icon: "fa-handshake"
      }

      ,
      sales: {
        label: "المبيعات", icon: "fa-chart-pie"
      }
    ...
    const externalLinks=[ {
      label: "الأوردرات",
        ...
        url: "https://almdrasa.com/wp-admin/edit.php?post_type=shop_order",
    }
    ...
    ,
      ...
      label: "الاشتراكات",
        icon: "fa-repeat",
        url: "https://almdrasa.com/wp-admin/edit.php?post_type=shop_subscription",
    }
    ...
    ,
      ...
      label: "الـoutsales",
        icon: "fa-chart-line",
        url: "https://almdrasa.com/wp-admin/admin.php?page=almdrasa-sales-outside-almdrasa",
    }
    ...
    ,
      ...
      label: "المستخدمين",
        icon: "fa-user-gear",
        url: "https://almdrasa.com/wp-admin/users.php",
    }
    ...
    ,
      ...
      label: "التسليمات",
        ...
        url: "https://almdrasa.com/wp-admin/edit.php?post_type=sfwd-assignment",
    }
    ...
    const renderActions=()=> {
      ...
      actionCard.innerHTML=` <div class="card-hint-overlay"><div class="hint-text">اضغط<span style="color: #fff; padding: 0 5px;">R</span>للتبديل</div></div><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 1rem;"><div style="display: flex; align-items: center; gap: 10px;"><h3 style="margin:0;"><i class="fa-solid fa-bolt gold-text"></i>إجراءات سريعة</h3></div><button class="radio-mini-btn edit-actions-btn" title="تعديل الإجراءات" style="width:30px; height:30px; margin:0!important; background:rgba(255,215,0,0.1); border-color:rgba(255,215,0,0.2);"><i class="fa-solid fa-pen-to-square" style="font-size:0.8rem; color:var(--metallic-gold);"></i></button></div><div class="quick-actions-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 0.8rem; animation: fadeIn 0.3s ease;">${
        showingExternal ? externalLinks .map((link)=> ` <a href="${link.url}" target="_blank" class="btn btn-glass" style="text-decoration: none;"> <i class="fa-solid ${link.icon}"></i> ${
          ...
          </a> `) .join("") : savedActions .map((key)=> {
            const action=allPossibleActions[key];
            if ( !action) return "";

            return `<button class="btn btn-glass" onclick="location.hash='#${key}'"> <i class="fa-solid ${action.icon}"></i> ${
              action.label
            }
      ...
      actionCard.querySelector(".edit-actions-btn").onclick=()=> {
        ...
        modal.innerHTML=` <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem;"><h3>تخصيص الإجراءات السريعة</h3><button class="btn btn-glass close-modal" style="padding:5px 10px;"><i class="fa-solid fa-xmark"></i></button></div><div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; flex:1; overflow-y:auto; padding-right:5px;">${
          Object.keys(allPossibleActions) .map((key)=> ` <div class="action-toggle-item ${
currentActions.includes(key) ? "active" : ""
          }
          ...
          border:1px solid ${
            currentActions.includes(key) ? "var(--metallic-gold)"
...
currentActions.includes(key) ? "#fff"
  ...
  </div><button class="btn btn-primary save-actions" style="margin-top:1.5rem; width:100%;">حفظ التغييرات</button>`;
...
const renderSalesCard=()=> {
  ...
  const timeStr=timestamp ? new Date(parseInt(timestamp)).toLocaleTimeString("ar-EG", {
      hour: "2-digit",
      minute: "2-digit",
    }
  ...
  salesCard.innerHTML=` <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 1.2rem;"><h3 style="margin:0;"><i class="fa-solid fa-chart-line gold-text"></i>ملخص المبيعات</h3><div style="display:flex; gap:10px; align-items:center;"><button class="dash-refresh-btn" id="dash-sales-refresh" title="تحديث البيانات"><i class="fa-solid fa-rotate"></i></button><div class="live-indicator"><span class="${
isOld ? "offline-dot" : "live-pulse"
}

"></span>
<span style="font-size:0.75rem; color:rgba(255,255,255,0.4)" id="dash-sales-time">${
  isOld ? "مخزنة: ": "مباشر: "
}
...
</span></div></div></div><div class="dashboard-sales-grid"><div class="dashboard-sales-item"><span class="dash-sales-label">مبيعات اليوم</span><span class="dash-sales-value">${
  today.totalEgp
}

</span></div><div class="dashboard-sales-item"><span class="dash-sales-label">مبيعات الأمس</span><span class="dash-sales-value">${
  yesterday.totalEgp
}

</span></div></div><div style="margin-top: 1.2rem; display: flex; gap: 0.8rem;"><div style="flex:1; background:rgba(255,215,0,0.05); padding:10px; border-radius:12px; border:1px solid rgba(255,215,0,0.1); text-align:center;"><div style="font-size:0.7rem; color:rgba(255,255,255,0.4); margin-bottom:4px;">عدد طلبات اليوم</div><div style="font-size:1.1rem; font-weight:700; color:var(--metallic-gold);">${
  today.orders
}

</div></div><button class="btn btn-glass" style="flex:1;" onclick="location.hash='#sales'"><i class="fa-solid fa-arrow-right"></i>التفاصيل </button></div>`;
...
setTimeout(async ()=> {
    ...
    =await import("./quran-radio.js");
    const radio=new QuranRadio();

    const radioContainer=document.createElement("div");
    radioContainer.className="glass-card premium-radio";

    radioContainer.innerHTML=` <div class="radio-header"> <div class="radio-main-info"> <div class="radio-icon"> <i class="fa-solid fa-mosque"></i> </div> <div class="radio-text"> <span class="radio-label">البث المباشر</span> <h3 class="radio-reciter-name">${
      radio.getCurrentReciter().name
    }

    </h3> </div> </div> <div class="radio-status-tag"> <span class="status-dot"></span> <span class="status-text">متصل</span> </div> </div> <div class="radio-visualizer"> <div class="waveform" id="radio-waveform" style="display:flex; align-items:flex-end; gap:8px; height:100%;"> ${
  ...
  const updateUI=(state, reciter)=> {
    ...
    if (muteBtnIcon) {
      muteBtnIcon.className=radio.muted ? "fa-solid fa-volume-xmark"
  ...
  radioContainer.querySelector("#search-reciter").onclick=()=> {
    ...
    modal.innerHTML=` <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem;"> <h3>اختر القارئ</h3> <button class="btn btn-glass close-modal" style="padding:5px 10px;"><i class="fa-solid fa-xmark"></i></button> </div> <input type="text" class="search-input" placeholder="ابحث عن قارئ أو إذاعة..." style="
...
<div class="reciter-list" style="flex:1; overflow-y:auto; padding-right:5px;"> ${
      radio.reciters .map((r, idx)=> ` <div class="reciter-item" data-idx="${idx}" style="
      ...
      if (freqData) {
        ...
        for (let i=0; i < barCount; i++) {
          ...
          if (bar) {
            ...
            bar.style.filter=`hue-rotate(${
                hueShift
              }

              deg) brightness(${
                1 + currentBarHeights[i] / 250
              }
          ...
          if (peak) {
            peak.style.transform=`translateY(${
                -peakValues[i] / 2
              }
...
return container;
  ...
  errDiv.innerHTML=`<h3 style="color:red">Dashboard Error: ${
    error.message
  }
